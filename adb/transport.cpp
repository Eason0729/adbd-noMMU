...
static void read_transport_thread(void* _t) {
    ...
    D("%s: starting read_transport thread on fd %d/%d, SYNC online (%d)", t->serial, t->fd.read_fd,
      t->fd.write_fd, t->sync_token + 1);
    p = get_apacket();
...
static void write_transport_thread(void* _t) {
    ...
    adb_thread_setname(
        android::base::StringPrintf("->%s", (t->serial != nullptr ? t->serial : "transport")));
    D("%s: starting write_transport thread, reading from fd %d", t->serial, t->fd.read_fd);

    for (;;) {
        if (read_packet(t->fd.read_fd, t->serial, &p)) {
            D("%s: failed to read apacket from transport on fd %d", t->serial, t->fd.read_fd);
            break;
        }
...
static void transport_registration_func(int _fd, unsigned ev, void* data) {
    ...
    if (m.action == 0) {
        D("transport: %s removing and free'ing %d/%d", t->serial, t->transport_socket.read_fd,
    ...
    if (t->connection_state != kCsNoPerm) {
        ...
        D("transport: %s channel pair: main(%d/%d) threads(%d/%d) starting", t->serial, a.read_fd,
          a.write_fd, b.read_fd, b.write_fd);

        t->transport_socket = a;  // main thread end: read+write
        t->fd = b;                // worker threads end: read+write

        fdevent_install(&(t->transport_fde), t->transport_socket.read_fd,
                        t->transport_socket.write_fd, transport_socket_events, t);
